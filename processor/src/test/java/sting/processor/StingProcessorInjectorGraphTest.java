...
public final class StingProcessorInjectorGraphTest extends AbstractStingProcessorTest {
    @Test
    public void eagerFlagPropagationInInjectors() throws Exception {
    ...
    @Test
    public void eagerInjectablesAddedWhenAddedViaIncludes() throws Exception {
    ...
    private void assertIndex(
        ...
        assertTrue(
                nodes.size() > index,
                "Attempting to lookup " + expectedId + " at index " + index + " but " + "there is only " + nodes.size()
                        + " nodes present.");
        final String id = nodes.getJsonObject(index).getString("id");
        assertEquals(
                id,
                expectedId,
                "Attempting to lookup " + expectedId + " at index " + index + " but " + "found id " + id);
    }
    ...
    @Test
    public void recursiveIncludesAreAllIncluded() throws Exception {
    ...
    @Test
    public void primitiveAndBoxedCollectionOutputRetainsBothBindings() throws Exception {
    ...
    @Test
    public void primitiveInputCanSatisfyBoxedDependency() throws Exception {
    ...
    @Test
    public void compoundNamesInIncludedProviders() throws Exception {
    ...
    @Test
    public void enclosingNamesInIncludedProviders() throws Exception {
    ...
    @Test
    public void flatEnclosingNamesInIncludedProviders() throws Exception {
    ...
    private void assertNodeWithIdPresent(@Nonnull final JsonArray nodes, @Nonnull final String id) {
        getNodeById(nodes, id);
    }

    @SuppressWarnings("SameParameterValue")
    private void assertNodeWithIdNotPresent(@Nonnull final JsonArray nodes, @Nonnull final String classname) {
    ...
    @Nonnull
    private JsonArray readInjectorGraph(@Nonnull final String filename) throws IOException {
        final JsonObject object =
                readJsonObject(fixtureDir().resolve("expected").resolve(filename));
        assertEquals(object.getString("schema"), "graph/1");
        return object.getJsonArray("nodes");
    }

    @Nonnull
    private JsonObject readJsonObject(@Nonnull final Path path) throws IOException {
