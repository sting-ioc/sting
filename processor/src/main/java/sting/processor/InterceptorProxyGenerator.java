...
final class InterceptorProxyGenerator
{
    ...
    for ( final var method : methods )
    {
      if ( shouldProxyMethod( serviceElement, method ) )
      {
        ...
        if ( emitted.add( signature ) )
        {
          ...
          builder.addMethod( buildServiceMethod( processingEnv,
                                                 proxy,
                                                 serviceElement,
                                                 method,
                                                 around,
  ...
  private static void emitAroundServiceMethod( @Nonnull final MethodSpec.Builder builder,
                                               ...
                                               @Nonnull final String invocationName )
  {
    ...
    suppressTypes.addAll( method.getParameters().stream().map( VariableElement::asType ).toList() );
    SuppressWarningsUtil.addSuppressWarningsIfRequired( processingEnv,
                                                        builder,
                                                        Collections.emptyList(),
                                                        suppressTypes );
  ...
  private static void emitAroundInvocationMethods( @Nonnull final ProcessingEnvironment processingEnv,
                                                   @Nonnull final TypeSpec.Builder builder,
                                                   @Nonnull final InterceptorProxyDescriptor proxy,
                                                   @Nonnull final ExecutableElement method,
                                                   @Nonnull final String invocationName )
  ...
  @Nonnull
  private static MethodSpec buildAroundInvocationMethod( @Nonnull final ProcessingEnvironment processingEnv,
                                                         @Nonnull final InterceptorProxyDescriptor proxy,
                                                         @Nonnull final ExecutableElement method,
                                                         @Nonnull final String invocationName,
                                                         final int index )
  {
    ...
    final var builder =
      MethodSpec.methodBuilder( methodName )
        ...
        .addParameter( ParameterSpec.builder( aroundInvocationArgumentsType( unmodifiableAnnotation ),
                                              "arguments",
                                              Modifier.FINAL )
                         .addAnnotation( GeneratorUtil.NONNULL_CLASSNAME )
                         .build() );
    propagateAroundInvocationNullability( builder, method );
    builder.addStatement( "assert null != $N && $L == $N.length",
                          "arguments",
    ...
    else
    {
      ...
      {
        builder.beginControlFlow( "try" );
        emitAroundInvocationStep( builder,
                                  proxy,
                                  method,
                                  interceptor,
                                  invocationName,
  ...
  private static AnnotationSpec findUnmodifiableAnnotation( @Nonnull final ProcessingEnvironment processingEnv )
  {
    ...
    return null == element ? null : AnnotationSpec.builder( ClassName.get( element ) ).build();
  }

  @Nonnull
  private static TypeName aroundInvocationReturnType( final boolean voidReturn,
                                                      @Nullable final AnnotationSpec unmodifiableAnnotation )
  {
    if ( voidReturn )
    {
    ...
    {
      return null == unmodifiableAnnotation ? ClassName.OBJECT : ClassName.OBJECT.annotated( unmodifiableAnnotation );
    }
  }
  ...
  private static void propagateAroundInvocationNullability( @Nonnull final MethodSpec.Builder builder,
                                                            @Nonnull final ExecutableElement method )
  {
  ...
  private static void emitAroundInvocationStep( @Nonnull final MethodSpec.Builder builder,
                                                @Nonnull final InterceptorProxyDescriptor proxy,
                                                @Nonnull final ExecutableElement method,
                                                ...
                                                final boolean voidReturn )
  {
    ...
    if ( null == aroundMethod )
    {
      if ( voidReturn )
      {
        builder.addStatement( "$N( $N )",
                              nextMethod,
                              "arguments" );
      }
      else
      {
        emitResultAssignment( builder,
                              method,
                              "result",
                              "$N( $N )",
                              nextMethod,
    ...
    else
    {
      ...
      if ( voidReturn )
      {
        builder.addStatement( "final $T $N = new $T( $N -> { $N( $N ); return null; }, $N )",
                              invocationType,
                              "invocation",
                              invocationType,
                              "nextArguments",
      ...
      {
        builder.addStatement( "final $T $N = new $T( $N -> $N( $N ), $N )",
                              invocationType,
                              "invocation",
                              invocationType,
                              "nextArguments",
      ...
      final var call =
        lifecycleCall( builder, proxy, method, interceptor, aroundMethod, null, null, "invocation", arguments );
      if ( voidReturn )
      {
      ...
      {
        emitResultAssignment( builder, method, "result", call.code(), call.args().toArray() );
      }
    }
  }

  @SuppressWarnings( "SameParameterValue" )
  private static void emitResultAssignment( @Nonnull final MethodSpec.Builder builder,
                                            @Nonnull final ExecutableElement method,
  ...
  private static void emitAroundTargetCall( @Nonnull final MethodSpec.Builder builder,
                                            @Nonnull final ExecutableElement method )
  {
    final var code = new StringBuilder();
    final var args = new ArrayList<>();
  ...
  private static void appendActiveArgumentList( @Nonnull final StringBuilder code,
  ...
  @SuppressWarnings( "SameParameterValue" )
  private static void emitInitialArguments( @Nonnull final MethodSpec.Builder builder,
  ...
  private static void emitPublicBoundaryCatches( @Nonnull final MethodSpec.Builder builder,
                                                 @Nonnull final ProcessingEnvironment processingEnv,
                                                 @Nonnull final ExecutableElement method )
  {
  ...
  @Nonnull
  private static String invocationMethodName( @Nonnull final String invocationName,
  ...
  private static void emitGenericLifecycle( @Nonnull final MethodSpec.Builder builder,
                                            ...
                                            @Nonnull final ArgumentState arguments )
  {
    final var call = lifecycleCall( builder,
                                    proxy,
                                    method,
                                    interceptor,
                                    lifecycleMethod,
  ...
  @Nonnull
  private static CodeFragment lifecycleCall( @Nonnull final MethodSpec.Builder builder,
                                             @Nonnull final InterceptorProxyDescriptor proxy,
                                             @Nonnull final ExecutableElement method,
                                             @Nonnull final InterceptorBindingDescriptor interceptor,
                                             @Nonnull final InterceptorMethodDescriptor lifecycleMethod,
  ...
  private record CodeFragment(@Nonnull String code, @Nonnull List<Object> args)
  {
  }

  private record ArgumentState(@Nonnull List<? extends VariableElement> parameters,
                               boolean required,
                               @Nullable String activeArgumentsName)
  {
    ...
    private void appendExpression( @Nonnull final StringBuilder code,
                                   @Nonnull final List<Object> args,
                                   @Nonnull final MethodSpec.Builder builder )
    {
      assert required;
