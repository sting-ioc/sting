...
public final class TransactionInterceptorTest {
    ...
    public void requiresNewSuspendsExistingTransaction() throws Throwable {
        ...
        final Object result =
                new RequiresNewTransactionInterceptor(manager).around(invocation(manager, "target", "ok"));
    ...
    public void uncheckedExceptionMarksExistingTransactionRollbackOnly() {
        ...
        final IllegalStateException exception = expectThrows(
                ...
                () -> new SupportsTransactionInterceptor(manager)
                        .around(failingInvocation(manager, "target", failure)));
    ...
    public void startedTransactionRollsBackAfterUncheckedFailure() {
        ...
        final IllegalStateException exception = expectThrows(
                IllegalStateException.class,
                () -> new RequiredTransactionInterceptor(manager)
                        .around(failingInvocation(manager, "target", failure)));
    ...
    public void startedTransactionRollsBackWhenMarkedRollbackAfterCheckedFailure() {
        ...
        final Exception exception = expectThrows(
                Exception.class,
                () -> new RequiredTransactionInterceptor(manager).around(invocation(manager, "target", failure, true)));
    ...
    public void suspendFailureIsWrapped() {
        ...
        final TransactionalException exception = expectThrows(
                TransactionalException.class,
                () -> new RequiresNewTransactionInterceptor(manager).around(invocation(manager, "target", null)));
    ...
    public void requiresNewResumesAfterRollbackFailure() {
        ...
        final TransactionalException exception = expectThrows(
                TransactionalException.class,
                () -> new RequiresNewTransactionInterceptor(manager)
                        .around(failingInvocation(manager, "target", failure)));
    ...
    public void notSupportedResumeFailureReplacesApplicationException() {
        ...
        final TransactionalException exception = expectThrows(
                TransactionalException.class,
                () -> new NotSupportedTransactionInterceptor(manager)
                        .around(failingInvocation(manager, "target", failure)));
    ...
    private static Invocation invocation(
            @Nonnull final FakeTransactionManager manager, @Nonnull final String trace, @Nullable final Object result) {
        return invocation(manager, trace, result, false);
    }

    @Nonnull
    private static Invocation invocation(
            @Nonnull final FakeTransactionManager manager,
    ...
    @Nonnull
    private static Invocation failingInvocation(
            @Nonnull final FakeTransactionManager manager,
            @Nonnull final String trace,
    ...
    private static final class FakeTransactionManager implements TransactionManager {
        @Nonnull
