...
public final class ProviderInjectableAnnotatedIncludedIntegrationTest extends AbstractIntegrationTest {
    ...
    public abstract static class Model1 extends BaseModel implements MyService {
        @Nonnull
    ...
    @Injectable
    @Typed(Model1.class)
    public static class Model1Impl extends Model1 {
        Model1Impl(@Nonnull final Model2 model2) {
            super(model2);
        }
    }

    @Injector(fragmentOnly = false, includes = Model1.class)
    public interface MyInjector {
        @Nonnull
        static MyInjector create() {
            return new ProviderInjectableAnnotatedIncludedIntegrationTest_Sting_MyInjector();
        }
